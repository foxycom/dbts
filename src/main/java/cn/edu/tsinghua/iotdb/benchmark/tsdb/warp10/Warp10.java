...
public class Warp10 implements IDatabase {
    ...
    public void cleanup() throws TsdbException {
        ...
        URI deleteAllUri = UrlBuilder.fromString(deleteUri)
    ...
    public Status precisePoint(Query query) {
        ...
        String selector = sensor.getSensorGroup().getName()
                + "{bike_id=" + bike.getName() + ",sensor_id=" + sensor.getName() + "}";
        URI uri = UrlBuilder.fromString(fetchUri)
    ...
    private Map<Sensor, Reading[]> transform(Batch batch) {
        ...
        Arrays.stream(gpsEntries).forEachOrdered(point -> {
        ...
        for (Sensor sensor : entries.keySet()) {
            ...
            for (int i = 0; i < points.length; i++) {
                ...
                sensorReadings[i] = new Reading(point.getTimestamp(), point, gpsLookupTable.get(point.getTimestamp()));
            }
