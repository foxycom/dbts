...
public class Clickhouse implements IDatabase {
    ...
    public void registerSchema(List<Bike> schemaList) throws TsdbException {
        try (Statement statement = connection.createStatement()) {
            ...
            statement.execute("CREATE TABLE IF NOT EXISTS bikes (bike_id String, owner_name String) "
    ...
    public Status precisePoint(Query query) {
        ...
        template.add("tableName", tableName).add("time", query.getStartTimestamp())
    ...
    public Status gpsPathScan(Query query) {
        ...
        template.add("tableName", tableName).add("bike", bike.getName())
                .add("start", query.getStartTimestamp()).add("end", query.getEndTimestamp());
        String debug = template.render();
        return executeQuery(debug);
    }
    ...
    public Status downsample(Query query) {
        ...
        template.add("tableName", tableName).add("sensor", sensor.getName())
                .add("bike", bike.getName()).add("start", query.getStartTimestamp())
    ...
    private String getInsertBikesSql(List<Bike> bikesList) {
        ...
        for (int i = 0; i < bikesList.size(); i++) {
            ...
            sqlBuilder.append("('").append(bike.getName()).append("', ")
                    .append("'").append(nameGenerator.getName()).append("')");
        }
        sqlBuilder.append(";");
    ...
    private String getCreateTableSql() {
        StringBuilder sqlBuilder = new StringBuilder();
        sqlBuilder.append("CREATE TABLE ").append(tableName).append(" (time UInt64, ").append("bike_id String");
        config.SENSORS.forEach(sensor -> {
            if (sensor.getFields().size() == 1) {
                sqlBuilder.append(", ").append(sensor.getName()).append(" Nullable(").append(sensor.getDataType()).append(")");
            } else {
                sensor.getFields().forEach(field ->
                        sqlBuilder.append(", ").append(field).append(" ").append(sensor.getDataType()));
            }
        });
        sqlBuilder.append(") ENGINE = MergeTree() PARTITION BY toYYYYMMDD(toDate(time/1000)) order by (bike_id, time);");
        return sqlBuilder.toString();
    }
