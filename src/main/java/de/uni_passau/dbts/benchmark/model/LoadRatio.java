package de.uni_passau.dbts.benchmark.model;
...
import de.uni_passau.dbts.benchmark.enums.LoadTypeEnum;
...
public class LoadRatio {
  ...
  public LoadRatio(
      double writeRatio,
      double randomInsertRatio,
      double simpleQueryRatio,
      double aggrQueryRatio,
      double updateRatio) {
    super();
    if (writeRatio < 0
        || randomInsertRatio < 0
  ...
  public static LoadRatio newInstanceByLoadType(LoadTypeEnum loadType) {
    ...
    switch (loadType) {
      ...
      case MUILTI:
        return new LoadRatio(
            config.WRITE_RATIO,
            config.RANDOM_INSERT_RATIO,
            config.SIMPLE_QUERY_RATIO,
            config.MAX_QUERY_RATIO,
            config.UPDATE_RATIO);
      default:
        break;
    }
